...
from implementation.module.sudokusolver import solver
from implementation.module.puzzle import puzzle, solution
import time
...
label = Label(root, text="Fill in the numbers and click submit", font=(myFont, 12, 'bold'), background="white",
              borderwidth=0).grid(row=0, column=1, columnspan=10, pady=15)
...
# Function to compare the player's solution with the correct puzzle solution
def compareSolution():
    changeSolveButtonState()
    changeSubmitButtonText()
    board = getValues()
...
# Function to Record the time used to successfully solve the puzzle
def recordSuccess():
    ...
    if len(timeTaken) == 0:
        ...
        if previousTime > newTime:
            results = "\nYou have improved from " + str(previousTime) + " to " + str(
...
# Function to Change the pause/play button state
...
# Function to Change the icon and command of the play/stop music button
...
pauseButton = tk.Button(text='Play Game', height=1, width=10, command=pause, background="#00b4d8", foreground="white",
                        borderwidth=0, font=myFont)
...
# Solve Button
solveBtn = Button(root, command=checkSolvable, text="Show solution", width=15, state=DISABLED, font=myFont,
                  borderwidth=0.5)
